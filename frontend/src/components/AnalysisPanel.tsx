...
export function AnalysisPanel() {
  ...
  // Check for book moves whenever position changes
  useEffect(() => {
    const checkBookMove = async () => {
      const isBook = await bookMovesDetector.isBookPosition(currentFen);
      setIsBookPosition(isBook);
      
      if (isBook) {
        const info = bookMovesDetector.getOpeningInfo(currentFen);
        setBookMoveInfo(info);
      } else {
  ...
  return (
    <div className="h-full flex flex-col bg-[#2b2926] text-gray-100 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        {/* Book Move Section - Always visible when position is in book */}
        {isBookPosition && bookMoveInfo && (
          <CollapsibleSection title="Opening Book" icon="📖" id="book-move" defaultOpen={true}>
            <div className="bg-gradient-to-br from-amber-900/30 to-orange-900/20 border border-amber-600/50 rounded-xl p-4">
              <div className="space-y-3">
                <div>
                  <div className="text-xs text-amber-300/70 uppercase tracking-wide mb-1">Opening Name</div>
                  <div className="text-xl font-bold text-amber-100">{bookMoveInfo.name}</div>
                </div>
                
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <div className="text-xs text-amber-300/70 uppercase tracking-wide mb-1">ECO Code</div>
                    <div className="text-lg font-mono font-semibold text-amber-200">{bookMoveInfo.eco}</div>
                  </div>
                  <div>
                    <div className="text-xs text-amber-300/70 uppercase tracking-wide mb-1">Status</div>
                    <div className="flex items-center gap-2">
                      <span className="inline-block w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
                      <span className="text-sm font-medium text-green-300">Theory</span>
                    </div>
                  </div>
                </div>
                ...
                {bookMoveInfo.moves && (
                  <div>
                    <div className="text-xs text-amber-300/70 uppercase tracking-wide mb-1">Move Sequence</div>
                    <div className="text-sm font-mono text-amber-200/90 bg-black/20 rounded px-3 py-2">
                      {bookMoveInfo.moves}
                    </div>
                  </div>
                )}
                ...
                <div className="pt-2 border-t border-amber-600/30">
                  <p className="text-xs text-amber-200/80 leading-relaxed">
                    This position is part of established opening theory. The moves shown are based on the 
                    Encyclopedia of Chess Openings (ECO) database and represent well-known theory.
                  </p>
                </div>
              </div>
            </div>
